...
Persamaan model adveksi dan difusi 1D dapat digunakan dan diterapkan pada bidang oseanografi di antaranya dalam analisis perubahan konsentrasi polutan, pemodelan intrusi laut, pemodelan pergerakkan radionuklida, pemodelan sebaran nutrien, suhu permukaan laut dan salinitas hingga sebaran limbah. Metode ini memiliki skema hasil yang lebih konsisten. Selain itu, metode ini lebih mudah untuk penyelesaian pendekatan implisit dan lebih cepat untuk simulasi pendekatan eksplisit serta dapat memodelkan polutan sesuai stabilitas perhitungan. Namun, metode ini tidak dapat menampilkan pergerakkan model berdasarkan arah.
## Modul 2 Adveksi Difusi 2D
Persamaan numerik adveksi difusi 2D dapat digunakan  untuk membuat skenario penyebaran konsentrasi polutan pada saat bulan kering atau basah. Penelitiannya dilakukan untuk menganalisis polutan sehingga kualitas air sungai dapat diketahui. Persamaan numerik tersebut dapat digunakan untuk melihat pergerakan polutan setiap waktu menggambarkan adanya penurunan konsentrasi polutan yang diakibatkan oleh adanya proses diffusi dari polutan yang terangkut oleh aliran air. Persamaan ini juga dapat digunakan dalam melakukan pemodelan air tanah, menghitung konsentrasi polutan pada pencemaran udara di lingkungan industry dan lain sebagainya.
## Modul 3 Hidrodinamika 1D
Persamaan model hidrodinamika 1D dapat diterapkan dalam bidang oseanografi seperti halnya pemodelan transport sedimen, proses-proses yang mempengaruhi sirkulasi dan pencampuran air, hingga pemodelan variasi tinggi muka air laut. Model hidrodinamika 1D memiliki karakteristik berupa hasil data yang seragam di seluruh penampang. Model ini sesuai untuk perairan dangkal dan gradien rendah. Namun, dalam pengolahannya metode ini memerlukan banyak data dan rawan _error_ pada perhitungan yang rumit.
## Modul 4 Hidrodinamika 2D
Model hidrodinamika 2D dapat diaplikasikan ke beberapa hal dalam bidang oseanografi seperti pemodelan gelombang dari aktivitas angin, pemodelan transport sedimen dan sedimentasi, pemodelan _oil spill_ atau tumpahan minyak hingga pemodelan _coastal dynamics_. Metode ini mampu memberikan data yang valid karena diperoleh dalam dua arah (x dan y atau x dan z). Data yang dihasilkan dari model hidrodinamika 2D lebih beragam dan dapat menunjukkan terjadinya anomali. Model ini sesuai apabila diterapkan dalam kondisi gradien curam. Namun, metode ini memerlukan data yang cukup banyak dan rentang waktu yang cukup lama untuk meilhat anomali serta terbatasnya data di beberapa wilayah.
